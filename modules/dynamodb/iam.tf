...
# Reservation API Service Role
data "aws_iam_policy_document" "reservation_service_policy" {
  statement {
    effect = "Allow"
    actions = [
      "dynamodb:GetItem",
  ...
  statement {
    ...
    resources = [
      "arn:aws:events:*:*:event-bus/${var.name}-reservation-events"
    ]
  }
  ...
  statement {
    ...
    resources = [
      "arn:aws:logs:*:*:log-group:/aws/ecs/reservation-api*",
      "arn:aws:logs:*:*:log-group:/aws/eks/reservation-api*"
    ]
  }
}

resource "aws_iam_role" "reservation_service_role" {
  ...
  assume_role_policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Condition = {
          StringEquals = {
            "*:sub" = "system:serviceaccount:*:reservation-api"
...
resource "aws_iam_policy" "reservation_service_policy" {
  name        = "${var.name}-reservation-api-service-policy"
  description = "Policy for reservation API service"
...
resource "aws_iam_role_policy_attachment" "reservation_service_policy_attachment" {
